        ...
        <tr role="row">
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 22%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">State<i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('state_name')"></i>
            </th>
    ...
    <x-slot name="tbody"> 
        @forelse($states as $state)
            <tr role="row" class="odd">
                <td class="sorting_1" tabindex="0">{{ $state->state_name }}</td>
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $state->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $state->id }})">{{ $state->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action> 
                    <a class="dropdown-item" href="{{ route('state.edit', ['state' => $state->id]) }}"><i
                            class="la la-edit"></i> Edit</a>
                            <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $state->id }})"><i
